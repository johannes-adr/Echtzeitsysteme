<script lang="ts">
    ...
    import Gojswindow from "../components/GoJsWindow.svelte"
    ...
    activeTab.subscribe(activeTabName=>{
        activeTabId = tabs.indexOf(activeTabName)
    })

    $: $activeTab = tabs[activeTabId];
    let isLight = browser?(localStorage.getItem("isLight") ?? "true") === "true":true;
    $:if(browser)localStorage.setItem("isLight",`${isLight}`);
</script>
...
<div class="flex flex-row h-full w-full gap-5 p-5">
    ...
    <div class="bg-base-300 controlWindow flex-grow flex flex-col rounded-xl overflow-hidden">
        <div class="bg-base-200 p-1 shadow-md z-10 w-full min-w-fit">
            <SlidingSelect bind:tabs bind:activeTab={activeTabId}></SlidingSelect>
        </div>
        {#if $activeTab === "Controller"}
            <Controller />
        {:else if $activeTab === "Editor"}
            <CsvEditor goLoad={loadData}/>
...
<label class="swap swap-rotate fixed top-0 right-0 z-20 scale-50">
    <input type="checkbox" class="theme-controller" value="light" bind:checked={isLight} />
